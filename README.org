...
[[https://github.com/MArpogaus/auto-tab-groups/blob/dev/COPYING][https://img.shields.io/github/license/MArpogaus/auto-tab-groups.svg?style=flat-square]]
...
Every project gets its own tab group, commands can open groups of
...
- Emacs 29.1 or later, including =tab-bar.el= and =project.el=.
- optionally =nerd-icons= for build in "eye-candy" mode

*** Installation

**** From MELPA

Once the package is on MELPA, install it with the built in package manager:
...
Every project gets its own tab group, and the modern tab style is one
mode away — the demo above runs on exactly this:

#+begin_src emacs-lisp
  (use-package auto-tab-groups
    :ensure t
    :init
    ;; A tab group per project, created on `project-switch-project'.
    (auto-tab-groups-project-mode)
    ;; The modern tab style.  Icons come from nerd-icons when it is
    ;; installed; plain strings work as well:
    ;; (auto-tab-groups-eyecandy-icons '(("HOME" . "⌂")))
    (auto-tab-groups-eyecandy-mode)
    (auto-tab-groups-mode))
#+end_src

Switch to a project with =C-x p p= and its tab group appears; kill the
project's buffers with =C-x p k= and the group closes.
...
=auto-tab-groups-echo-mode= prints a message in the echo area whenever
a tab group is created, switched to or closed.
...
This companion mode integrates with =project.el=, similar to
[[https://github.com/fritzgrabo/project-tab-groups][project-tab-groups.el]].  Enable it with:

#+begin_src emacs-lisp
  (auto-tab-groups-project-mode)
#+end_src

Every project then gets a tab group named after it, created when you
switch to the project and closed by =project-kill-buffers=.  The name
comes from =auto-tab-groups-project-group-name=, which is also usable
as the group name function in your own
=auto-tab-groups-create-commands= rules: it takes whatever the command
returned, which may be a directory, a buffer or the name of a known
project.  The mode offers no options of its own.

*** auto-tab-groups-eyecandy

The =auto-tab-groups-eyecandy-mode= minor mode styles the tab bar and
...
| =auto-tab-groups-eyecandy-default-icon=                       | string, plist | Icon for groups without a match.         |
| =auto-tab-groups-eyecandy-tab-bar-format=                     | list          | List of tab bar elements to use.         |
| =auto-tab-groups-eyecandy-tab-bar-group-name-format-function= | function      | Custom function to alter the group name. |

=auto-tab-groups-eyecandy-icons= is an alist where:

- CAR: regular expression, matched against the tab group name.
- CDR: either a string shown as is, or a nerd-icons specification, a
  plist with the keys =:style= and =:icon= (requires [[https://github.com/rainstormstudio/nerd-icons.el][nerd-icons]]).

The first matching entry wins.  Icons the frame cannot draw fall back
to plain characters on their own.
