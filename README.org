...
=auto-tab-groups-eyecandy-mode= styles the tab bar.  The tab bar as
Emacs draws it, with the groups in it:

[[file:img/tab-bar-plain.png]]

And the same four groups with the mode on, in the configuration
example above:

[[file:img/tab-bar.png]]

The current group is at the left with its tab.  The other groups
follow, with an icon on each, and the button at the end makes a new
group.  The prefix of the project group is gone, because the example
gives =auto-tab-groups-eyecandy-group-name-function= a function that
takes it off.
...
| =auto-tab-groups-eyecandy-group-name-function=                 | function      | a function that changes the group name  |
...
The eyecandy mode works in =emacs -nw=.  Two elements of the bar
behave differently there, and the mode takes care of both:

- A spacer is a plain space.  A terminal draws no part of a cell.
- The bar has no menu button.  A terminal paints the row of the bar in
  the redisplay that turns the bar on.  With the menu button on the
  row, a tab that changes before that redisplay leaves the row blank
  for the rest of the session.  =M-x menu-bar-open= (=F10=) opens the
  same menu.
...
checkdoc, package-lint and the batch tests.  =make format= indents every
Lisp file, which is =tools/indent.el= and nothing more than the
indentation =indent-region= gives Lisp: no form is rewritten and no line
reflowed.
