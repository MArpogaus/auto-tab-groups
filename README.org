...
Any elisp function defined in =auto-tab-groups-create-commands= will create a new group or switch to an existing one with the given name.
Any elisp function defined in =auto-tab-groups-close-commands= will close the corresponding group and all tabs belonging to it.
...
All cammands fot managing tabs and tab groups are provided by =tab-bar.el=.
