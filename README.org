...
[[https://melpa.org/#/auto-tab-groups][https://melpa.org/packages/auto-tab-groups-badge.svg]]
...
#+begin_src emacs-lisp
  (use-package auto-tab-groups
    :vc (:url "https://github.com/MArpogaus/auto-tab-groups" :rev :newest)
...
- CDR: the name of the group, or a function that returns the name, or
  a plist with =:tab-group-name= and the flag of that kind of rule.

For a create command the package runs the command first and gives the
result to the name function.  With =:before-command= it makes the
group first, and calls the name function with no argument.

For a close command the package closes the group if the command
returns a value.  With =:always-close= it closes the group in each
case.

The two flags carry different names because they decide different
things: =:before-command= decides when the group is made and whether
the name function is handed anything, =:always-close= decides whether
the command's answer may call the closing off.
